...
func TestMinimal_StoreRetrieve(t *testing.T) {
	tempDir := t.TempDir()
	...
	err := cluster.StoreChunk("test-chunk", testData)
	if err != nil {
		t.Fatalf("StoreChunk failed: %v", err)
	}
	t.Log("✅ StoreChunk works")
	...
	// Retrieve the data
	retrieved, err := cluster.GetChunk("test-chunk")
	if err != nil {
		t.Fatalf("GetChunk failed: %v", err)
	}

	if string(retrieved) != string(testData) {
		t.Fatalf("Data mismatch: got %q, want %q", string(retrieved), string(testData))
	}

	t.Log("✅ GetChunk works")
}
