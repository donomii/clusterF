...
<body>
    <div class="container">
        <div class="panel">
            <h2>CRDT Browser</h2>
            <div class="tools">
                <input id="prefix" placeholder="Prefix, e.g. partitions or chunks">
...
func (c *Cluster) handleCRDTListAPI(w http.ResponseWriter, r *http.Request) {
    ...
    if prefix == "" {
        ...
        if after != "" {
            for i, it := range items {
                if it["name"] > after { start = i; break }
            }
        }
        end := start + limit
        ...
        if end < len(items) { resp["next_after"] = items[end-1]["name"] }
    ...
    if prefix == "partitions" || strings.HasPrefix(prefix, "partitions/") {
        ...
        if strings.HasPrefix(rem, "/") { rem = rem[1:] }
        ...
        paginate := func(names []string) (page []string, hasMore bool, next string) {
            ...
            start := 0
            if after != "" {
                for i := range names { if names[i] > after { start = i; break } }
            }
            end := start + limit
            if end > len(names) { end = len(names) }
            page = names[start:end]
            if end < len(names) { hasMore = true; next = names[end-1] }
            return
        }
        ...
        if rem == "" || len(rem) < 3 {
            ...
            if hasMore { resp["next_after"] = next }
            w.Header().Set("Content-Type", "application/json")
            json.NewEncoder(w).Encode(resp)
            return
        }
        if len(rem) == 3 {
            // p000..p009
            base := rem
            if len(base) != 3 { http.Error(w, "bad prefix", http.StatusBadRequest); return }
            names := make([]string, 0, 10)
            for i := 0; i <= 9; i++ { names = append(names, fmt.Sprintf("%s%d", base, i)) }
            page, hasMore, next := paginate(names)
            items := make([]map[string]interface{}, 0, len(page))
            for _, g := range page {
                cnt := c.countCRDTKeys("partitions/" + g)
        ...
        for _, dp := range dps {
            ...
            if !strings.HasPrefix(rest, rem) { continue }
            if strings.Contains(rest[len(rem):], "/") { continue }
            keySet[rest] = struct{}{}
        }
        names := make([]string, 0, len(keySet))
        for n := range keySet { names = append(names, n) }
        sort.Strings(names)
        start := 0
        if after != "" {
            for i := range names { if names[i] > after { start = i; break } }
        ...
        for _, n := range names[start:end] { out = append(out, map[string]string{"name": n, "type": "key"}) }
        resp := map[string]interface{}{"prefix": prefix, "items": out, "has_more": end < len(names)}
        if end < len(names) { resp["next_after"] = names[end-1] }
        w.Header().Set("Content-Type", "application/json")
        json.NewEncoder(w).Encode(resp)
        return
    }
    ...
    for _, dp := range dps {
        ...
        if strings.HasPrefix(rest, "/") { rest = rest[1:] }
        if rest == "" { // exact key equals prefix; skip in listing
            continue
        }
        if i := strings.IndexByte(rest, '/'); i >= 0 {
            seg := rest[:i]
            if seg != "" { dirSet[seg] = struct{}{} }
        } else {
            keySet[rest] = struct{}{}
        }
    }
    items := make([]struct{ Name, Type string }, 0, len(dirSet)+len(keySet))
    for name := range dirSet { items = append(items, struct{ Name, Type string }{name, "dir"}) }
    for name := range keySet { items = append(items, struct{ Name, Type string }{name, "key"}) }
    sort.Slice(items, func(i, j int) bool {
        if items[i].Name == items[j].Name { return items[i].Type < items[j].Type }
    ...
    if after != "" {
        for i := range items {
            if items[i].Name > after { start = i; break }
        }
    }
    end := start + limit
    if end > len(items) { end = len(items) }
    ...
    for _, it := range page { out = append(out, map[string]string{"name": it.Name, "type": it.Type}) }
...
func (c *Cluster) handleCRDTSearchAPI(w http.ResponseWriter, r *http.Request) {
    ...
    start := 0
    if after != "" {
        for i := range keys { if keys[i] > after { start = i; break } }
    }
    end := start + limit
    ...
    if end < len(keys) { resp["next_after"] = keys[end-1] }
    w.Header().Set("Content-Type", "application/json")
    json.NewEncoder(w).Encode(resp)
}
