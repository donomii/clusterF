...
func main() {
	...
	storageMajor := flag.String("storage-major", "mmapsingle", "Storage format major (ensemble or bolt or sqlite or rawfile)")
	storageMinor := flag.String("storage-minor", "", "Storage format minor (bolt or sqlite or rawfile)")
	...
	for batch := 0; batch < nodeCount; batch += batchSize {
		...
		for i := batch; i < end; i++ {
			...
			go func(index int) {
				...
				if _, err := node.FileSystem.StoreFileWithModTimeAndClusterUpdate(context.TODO(), fmt.Sprintf("/demo-%03d.txt", index), []byte(demoContent), "text/plain", demoTimestamp); err != nil {
